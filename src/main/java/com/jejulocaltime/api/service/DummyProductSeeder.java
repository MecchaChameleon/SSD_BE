...
public class DummyProductSeeder {
    ...
    public void refresh() {
        ...
        for (int i = 0; i < SPECS.size(); i++) {
            DummySpec spec = SPECS.get(i);
            int imageSeed = i;
            Product product = productRepository
                    .findBySellerProfileIdAndName(sellerProfile.getId(), spec.name())
                    .orElseGet(() -> createProduct(sellerProfile, spec, imageSeed, now, reservationCloseAt));
    ...
    private Product createProduct(SellerProfile sellerProfile, DummySpec spec, int index,
        ...
        productImageRepository.save(new ProductImage(
                saved.getId(),
                "https://picsum.photos/seed/jeju-dummy-" + index + "/800/600",
                0));
