...
public class FrontendApiService {
    ...
    @Transactional public ReservationResponse cancel(Long userId,Long id,String reason){var old=reservation(userId,id,false);if(!"REQUESTED".equals(old.status()))throw new ResponseStatusException(CONFLICT,"대기 중인 예약만 취소할 수 있습니다.");jdbc.update("UPDATE reservation SET status='CANCELED',cancel_reason=?,canceled_at=now(),updated_at=now() WHERE id=?",reason,id);return reservation(userId,id,false);}
    @Transactional public void hide(Long userId,Long id){var old=reservation(userId,id,false);if(!"REJECTED".equals(old.status())&&!"PAID".equals(old.paymentStatus()))throw new ResponseStatusException(CONFLICT,"거절되었거나 결제가 완료된 예약 내역만 삭제할 수 있습니다.");jdbc.update("UPDATE reservation SET hidden_by_buyer=true,updated_at=now() WHERE id=? AND user_id=?",id,userId);}
    ...
    public DashboardResponse dashboard(Long userId,LocalDate date){Long sp=sellerProfileId(userId);long req=count("SELECT count(*) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='REQUESTED' AND r.requested_at::date=?",sp,date);long app=count("SELECT count(*) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='APPROVED' AND r.requested_at::date=?",sp,date);long no=count("SELECT count(*) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='NO_SHOW' AND r.requested_at::date=?",sp,date);long daily=count("SELECT COALESCE(sum(r.unit_price*r.quantity),0) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='COMPLETED' AND r.payment_status='PAID' AND r.completed_at::date=?",sp,date);long products=count("SELECT count(*) FROM product WHERE seller_profile_id=?",sp);return new DashboardResponse(date,new ReservationCounts(req,app,no),daily,daily,products);}
    public SalesReportResponse sales(Long userId,LocalDate start,LocalDate end,String sort){Long sp=sellerProfileId(userId);String order="REVENUE_ASC".equals(sort)?"revenue ASC":"revenue DESC";var items=jdbc.query("SELECT p.id,p.name,sum(r.quantity) quantity,sum(r.unit_price*r.quantity) revenue FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='COMPLETED' AND r.payment_status='PAID' AND r.completed_at::date BETWEEN ? AND ? GROUP BY p.id,p.name ORDER BY "+order,(r,n)->new SalesItem(r.getLong(1),r.getString(2),r.getLong(3),r.getLong(4)),sp,start,end);long revenue=items.stream().mapToLong(SalesItem::revenue).sum(),qty=items.stream().mapToLong(SalesItem::quantity).sum();return new SalesReportResponse(start,end,revenue,revenue,qty,items);}
    public PageResponse<SalesHistoryItem> salesHistory(Long userId,LocalDate start,LocalDate end,int page,int size){
        ...
        String from=" FROM reservation r JOIN product p ON p.id=r.product_id JOIN users u ON u.id=r.user_id WHERE p.seller_profile_id=? AND r.status='COMPLETED' AND r.payment_status='PAID' AND r.completed_at::date BETWEEN ? AND ? ";
        var items=jdbc.query("SELECT r.id reservation_id,p.id product_id,p.name product_name,u.id buyer_id,u.nickname buyer_nickname,r.quantity,r.unit_price,r.total_amount,r.completed_at"+from+"ORDER BY r.completed_at DESC,r.id DESC LIMIT ? OFFSET ?",(r,n)->new SalesHistoryItem(r.getLong("reservation_id"),r.getLong("product_id"),r.getString("product_name"),r.getLong("buyer_id"),r.getString("buyer_nickname"),r.getInt("quantity"),r.getInt("unit_price"),r.getInt("total_amount"),time(r,"completed_at")),sp,start,end,safeSize,safePage*safeSize);
