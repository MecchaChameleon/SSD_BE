...
public class FrontendApiService {
    ...
    public PageResponse<PurchaseResponse> sellerPayments(Long userId,String status,LocalDate date,int page,int size){Long sp=sellerProfileId(userId);String dbStatus=dbPurchaseStatus(status);String where=" WHERE p.seller_profile_id=? AND r.payment_status IN ('PAID','REFUNDED') "+(dbStatus==null?"":"AND r.status=? ")+(date==null?"":"AND r.requested_at::date=? ");List<Object>a=new ArrayList<>();a.add(sp);if(dbStatus!=null)a.add(dbStatus);if(date!=null)a.add(date);List<Object>countArgs=new ArrayList<>(a);a.add(size);a.add(page*size);var list=jdbc.query(purchaseSql()+where+" ORDER BY r.requested_at DESC LIMIT ? OFFSET ?",purchaseMapper,a.toArray());long total=count("SELECT count(*) FROM reservation r JOIN product p ON p.id=r.product_id"+where,countArgs.toArray());return page(list,page,size,total);}
    ...
    public DashboardResponse dashboard(Long userId,LocalDate date){Long sp=sellerProfileId(userId);long pending=count("SELECT count(*) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='REQUESTED' AND r.payment_status='PAID' AND r.requested_at::date=?",sp,date);long accepted=count("SELECT count(*) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='COMPLETED' AND r.payment_status='PAID' AND r.completed_at::date=?",sp,date);long refunded=count("SELECT count(*) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='REJECTED' AND r.payment_status='REFUNDED' AND r.rejected_at::date=?",sp,date);long daily=count("SELECT COALESCE(sum(r.unit_price*r.quantity),0) FROM reservation r JOIN product p ON p.id=r.product_id WHERE p.seller_profile_id=? AND r.status='COMPLETED' AND r.payment_status='PAID' AND r.completed_at::date=?",sp,date);long products=count("SELECT count(*) FROM product WHERE seller_profile_id=?",sp);return new DashboardResponse(date,new PaymentCounts(pending,accepted,refunded),daily,daily,products);}
